...
for (i in 1:nrow(p_combinations)) {
  ...
  cat(sprintf("\n========== ABC Start p0 = %.2f, p1 = %.2f ==========\n", p0, p1))
  
  cliRequirement <- list(p0 = p0, p1 = p1, alpha = 0.1, beta = 0.1)
  results <- list()
  
  for (seed in seeds) {
    cat(sprintf("Running ABC: p0 = %.2f, p1 = %.2f, seed = %d\n", p0, p1, seed))
    result_seed <- list()
    
    maxIter_single <- iter_combinations$single$maxIter
    result_seed[[paste0("single_opt_", maxIter_single)]] <- run_abc_single_stage(seed, 0, cliRequirement, nStage, maxIter_single, nMaxRange = nMaxRange)
    result_seed[[paste0("single_min_", maxIter_single)]] <- run_abc_single_stage(seed, 1, cliRequirement, nStage, maxIter_single, nMaxRange = nMaxRange)
    ...
    for (combo in iter_combinations$two_stage) {
      ...
      result_seed[[paste0("opt_", maxIter1, "_", maxIter2)]] <-
        run_abc_two_stage(seed, 1, 0, cliRequirement, nStage, maxIter1, maxIter2, nMaxRange = nMaxRange, upper2_is_fixed = FALSE)
      
      result_seed[[paste0("min_", maxIter1, "_", maxIter2)]] <-
        run_abc_two_stage(seed, 1, 1, cliRequirement, nStage, maxIter1, maxIter2, nMaxRange = nMaxRange, upper2_is_fixed = TRUE)
    }
...
saveRDS(all_results, file = "results/compare_results_abc.rds")

# === 載入結果 ===
all_results <- readRDS("results/compare_results_abc.rds")
