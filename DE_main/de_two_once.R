...
source("run/run_de_stage.R")  # 上面那段存成此檔案
...
for (i in 1:nrow(p_combinations)) {
  ...
  cli <- list(p0 = p0, p1 = p1, alpha = 0.1, beta = 0.1)
  cat(sprintf("=== DE Two-Stage: p0 = %.2f, p1 = %.2f ===\n", p0, p1))
  
  opt <- run_de_two_stage(seed, 1, 0, cli, nStage, maxIter1, maxIter2, nMaxRange, upper2_is_fixed = FALSE)
  min <- run_de_two_stage(seed, 1, 1, cli, nStage, maxIter1, maxIter2, nMaxRange, upper2_is_fixed = TRUE)
  
  results[[i]] <- list(p0 = p0, p1 = p1, optimal = opt, minimax = min)
}
