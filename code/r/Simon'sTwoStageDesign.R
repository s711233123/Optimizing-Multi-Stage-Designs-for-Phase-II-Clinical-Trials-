...
# PSO優化，使用globpso
pso_result <- globpso(objFunc = pso_fitness, lower = lower_bounds, upper = upper_bounds, PSO_INFO = getPSOInfo(maxIter = 1000, nSwarm = 100))

# 打印結果並四捨五入到整數
rounded_par <- round(pso_result$par)
round_val<-round(pso_result$val, 1)
print(rounded_par)
print(round_val)
