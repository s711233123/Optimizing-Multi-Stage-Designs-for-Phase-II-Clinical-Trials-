...
upper_bound <- c(n - 1, n - 1, n - 1, n - 1)

# 儲存結果的資料框
results <- data.frame(alpha = numeric(0), beta = numeric(0), r1 = numeric(0), n1 = numeric(0), r2 = numeric(0), n2 = numeric(0), EN = numeric(0))
...
for (ab in alpha_beta_pairs) {
  ...
  res_c <- globpso(objFunc = pso_fitness, lower = lower_bound, upper = upper_bound, PSO_INFO = getPSOInfo(maxIter = 300, nSwarm = 100), p1 = p1, p0 = p0, alpha = alpha, beta = beta)
...
colnames(results) <- c("alpha", "beta", "r1", "n1", "r2", "n2", "EN")
